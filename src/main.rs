...
fn get_session_key() -> String {
	...
	for _ in 0..10 { result += &(((rand::random::<f64>()*9.0 + 1.0) as i64).to_string()); }
	return result;
}
...
fn get_hash_str() -> String {
	...
	for _ in 0..5 { li += &(((rand::random::<f64>()*6.0 + 1.0) as i64).to_string()); }
	return li;
}
...
fn calc_hash(session_key: &String, val :&i64) -> String {
    ...
    if *val == 1 {
        ...
        let mut num  = 0;
        for i in 1..9 { num += (&session_key[i..i+1]).parse::<i32>().unwrap() + 41; }
        ...
        for i in 0..session_key.len() {
            ...
            if !cha.is_digit(10) {
                num += cha as u32;
            } else { num += cha.to_digit(10).unwrap(); }

        }

        return num.to_string()
    }
    else { return (session_key.parse::<i64>().unwrap() + val).to_string(); }
}
            ...
            while count < max_count {
                ...
                match stream.read(&mut res_key) {
                    Ok(_) => {
                        ...
                        else { println!("Incorrect key: {}", inpkey); break; }
                    },
                    Err(e) => { println!("Failed to receive data: {}", e); break; }
                }
                count+=1;
...
fn main() {
    ...
    if input.find(':') == None { run_server(input) }
    else { run_client(input) };
}